#include <stdint.h>
#include <string.h>

const uint8_t Sbox[256] = {
    
};

static uint32_t rotl32(uint32_t x, int n) { return (x << n) | (x >> (32 - n)); }
static uint32_t sm4_t(uint32_t x) {
    uint32_t r = 0;
    r |= (uint32_t)Sbox[(x >> 24) & 0xFF] << 24;
    r |= (uint32_t)Sbox[(x >> 16) & 0xFF] << 16;
    r |= (uint32_t)Sbox[(x >> 8) & 0xFF] << 8;
    r |= (uint32_t)Sbox[x & 0xFF];
    return r;
}
static uint32_t sm4_l(uint32_t x) { return x ^ rotl32(x,2)^rotl32(x,10)^rotl32(x,18)^rotl32(x,24); }
static uint32_t sm4_lp(uint32_t x) { return x ^ rotl32(x,13)^rotl32(x,23); }

void sm4_key_expand(const uint8_t key[16], uint32_t rk[32]) {
    const uint32_t FK[4]={0xA3B1BAC6,0x56AA3350,0x677D9197,0xB27022DC};
    const uint32_t CK[32] = {0x00070E15,0x1C232A31,0x383F464D,0x545B6269,0x70777E85,0x8C939AA1,
        0xA8AFB6BD,0xC4CBD2D9,0xE0E7EEF5,0xFC030A11,0x181F262D,0x343B4249,0x50575E65,0x6C737A81,
        0x888F969D,0xA4ABB2B9,0xC0C7CED5,0xDCE3EAF1,0xF8FF060D,0x141B2229,0x30373E45,0x4C535A61,
        0x686F767D,0x848B9299,0xA0A7AEB5,0xBCC3CAD1,0xD8DFE6ED,0xF4FB0209,0x10171E25,0x2C333A41,
        0x484F565D,0x646B7279};
    uint32_t K[4];
    K[0]= (key[0]<<24)|(key[1]<<16)|(key[2]<<8)|key[3];
    K[1]= (key[4]<<24)|(key[5]<<16)|(key[6]<<8)|key[7];
    K[2]= (key[8]<<24)|(key[9]<<16)|(key[10]<<8)|key[11];
    K[3]= (key[12]<<24)|(key[13]<<16)|(key[14]<<8)|key[15];
    K[0]^=FK[0]; K[1]^=FK[1]; K[2]^=FK[2]; K[3]^=FK[3];
    for(int i=0;i<32;i++){
        uint32_t t=K[1]^K[2]^K[3]^CK[i];
        uint32_t rk_i=K[0]^sm4_lp(sm4_t(t));
        rk[i]=rk_i;
        K[0]=K[1]; K[1]=K[2]; K[2]=K[3]; K[3]=rk_i;
    }
}

void sm4_encrypt_block(const uint8_t in[16], uint8_t out[16], const uint32_t rk[32]) {
    uint32_t X[4];
    X[0]=(in[0]<<24)|(in[1]<<16)|(in[2]<<8)|in[3];
    X[1]=(in[4]<<24)|(in[5]<<16)|(in[6]<<8)|in[7];
    X[2]=(in[8]<<24)|(in[9]<<16)|(in[10]<<8)|in[11];
    X[3]=(in[12]<<24)|(in[13]<<16)|(in[14]<<8)|in[15];
    for(int i=0;i<32;i++){
        uint32_t t=X[1]^X[2]^X[3]^rk[i];
        uint32_t tmp=X[0]^sm4_l(sm4_t(t));
        X[0]=X[1]; X[1]=X[2]; X[2]=X[3]; X[3]=tmp;
    }
    out[0]=(X[3]>>24)&0xFF; out[1]=(X[3]>>16)&0xFF; out[2]=(X[3]>>8)&0xFF; out[3]=X[3]&0xFF;
    out[4]=(X[2]>>24)&0xFF; out[5]=(X[2]>>16)&0xFF; out[6]=(X[2]>>8)&0xFF; out[7]=X[2]&0xFF;
    out[8]=(X[1]>>24)&0xFF; out[9]=(X[1]>>16)&0xFF; out[10]=(X[1]>>8)&0xFF; out[11]=X[1]&0xFF;
    out[12]=(X[0]>>24)&0xFF; out[13]=(X[0]>>16)&0xFF; out[14]=(X[0]>>8)&0xFF; out[15]=X[0]&0xFF;
}
