...
void aes_key_expand(const uint8_t key[16], uint8_t round_keys[11][16]) {
    ...
    for (int i = 0; i < 11; i++) {
        for (int j = 0; j < 4; j++) {
            uint32_t val = w[4*i + j];
            round_keys[i][4*j]   = (val >> 24) & 0xFF;
            round_keys[i][4*j+1] = (val >> 16) & 0xFF;
            round_keys[i][4*j+2] = (val >> 8) & 0xFF;
            round_keys[i][4*j+3] = val & 0xFF;
        }
    }
}
...
void aes_encrypt_block(const uint8_t in[16], uint8_t out[16], const uint8_t round_keys[11][16]) {
    ...
    for (int round = 1; round < 10; round++) {
        ...
        tmp = state[3][0]; state[3][0] = state[3][3]; state[3][3] = state[3][2]; state[3][2] = state[3][1]; state[3][1] = tmp;
        // MixColumns
        for (int c = 0; c < 4; c++) {
    ...
    // ShiftRows
    uint8_t tmp2;
    tmp2 = state[1][0]; state[1][0] = state[1][1]; state[1][1] = state[1][2]; state[1][2] = state[1][3]; state[1][3] = tmp2;
    tmp2 = state[2][0]; state[2][0] = state[2][2]; state[2][2] = tmp2;
    tmp2 = state[2][1]; state[2][1] = state[2][3]; state[2][3] = tmp2;
    tmp2 = state[3][0]; state[3][0] = state[3][3]; state[3][3] = state[3][2]; state[3][2] = state[3][1]; state[3][1] = tmp2;

    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            state[i][j] ^= round_keys[10][i*4 + j];
